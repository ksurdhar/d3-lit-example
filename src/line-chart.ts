...
export class LineChart extends LitElement {
  ...
  @state()
  protected linePath:
    | Selection<SVGPathElement, ConnectionData[], HTMLElement, any>
    | undefined;
  ...
  constructor() {
    super();
    this.data = [
      { time: new Date(2023, 5, 15, 9, 0), yUnit: 12 },
      { time: new Date(2023, 5, 15, 9, 5), yUnit: 20 },
      { time: new Date(2023, 5, 15, 9, 10), yUnit: 10 },
      { time: new Date(2023, 5, 15, 9, 15), yUnit: 11 },
      { time: new Date(2023, 5, 15, 9, 20), yUnit: 30 },
      { time: new Date(2023, 5, 15, 9, 25), yUnit: 8 },
      { time: new Date(2023, 5, 15, 9, 30), yUnit: 20 },
      { time: new Date(2023, 5, 15, 9, 35), yUnit: 16 },
      { time: new Date(2023, 5, 15, 9, 40), yUnit: 25 },
    ];
  }

  randomizeData() {
    const newData = Array.from({ length: 9 }, (_, idx) => {
      const yUnit = Math.floor(Math.random() * 30);
  ...
  protected firstUpdated(_changedProperties: PropertyValues) {
    ...
    svg
      ...
      .attr('y2', yScale(0))
      .attr('x2', 0)
      .attr('y1', yScale(d3.max(this.data, (d) => d.yUnit) as number))
      .selectAll('stop')
      .data([
